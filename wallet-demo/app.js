...
if (!demoRuntime?.APP_VERSION || !demoRuntime.PORT_CONTRACT || !demoRuntime.WALLET_CHAIN_OPTIONS || !demoRuntime.ASSET_CATALOG || !demoRuntime.EXTENSION_CATALOG || !demoRuntime.MESSENGER_MESSAGES || !demoRuntime.CONTACT_FIXTURES || !demoRuntime.createInitialState || !demoRuntime.createNavigationSession || !demoRuntime.createMockWalletGateway || !demoRuntime.createMockTelemetryGateway || !demoRuntime.createMockExtensionGateway || !demoRuntime.createMockMessengerGateway || !demoRuntime.createMockContactsGateway) {
...
const extensionGateway = demoRuntime.createMockExtensionGateway();
...
function clearExternalReviewHandoffs() {
  state.extensionWalletReviewHandoff = null;
...
function hasSelectedWalletContext() {
  return Boolean(state.selectedWalletId)
    && !["settings", "telemetry", "extensions", "messenger", "contacts", "data-storage", "about"].includes(state.view);
}
...
function routeFromCurrentLegacyState() {
  if (["extensions", "messenger", "contacts", "data-storage", "about"].includes(state.view)
...
function legacyStateForRoute(routeId) {
  ...
  if (routeId.startsWith("extensions.")) {
    return {
      view: "extensions",
      extensionSection: routeId.split(".").at(-1),
      extensionScreen: "list",
      extensionSelectedId: null,
      extensionReviewConnectionId: null,
      extensionReviewValidationError: null,
      extensionReviewAcknowledgements: {
...
function extensionWalletReviewNotice() {
  const handoff = state.extensionWalletReviewHandoff;
  if (!handoff || handoff.target.routeId !== state.activeRoute) return "";
  const descriptor = demoRuntime.extensionDescriptor(handoff.source.descriptorId);
  return `<div class="capability-note extension-wallet-handoff" data-extension-wallet-handoff="${escapeHtml(handoff.handoffId)}">${icon("shield")}<span><strong>Prepared from ${escapeHtml(descriptor?.label || "bounded extension intent")}</strong><small>Only a typed, immutable prefill crossed into Wallet. Confirm the recipient, amount, item, and fee here; no wallet object changed during handoff.</small></span></div>`;
}
...
function walletSendView() {
  ...
  return `<div class="view-enter workspace-layout send-workspace-layout"><aside class="context-rail">${sendFamilyContextNav(draft.family)}</aside><div class="workspace-panel send-view">${extensionWalletReviewNotice()}${messengerWalletReviewNotice()}${contactWalletReviewNotice()}${frame}</div></div>`;
}
...
function walletView() {
  ...
  return `<div class="view-enter workspace-layout wallet-assets-layout"><aside class="context-rail">${walletContextNav()}</aside><div id="wallet-sections-panel" class="workspace-panel" role="tabpanel" aria-labelledby="wallet-section-${state.walletSection}">${extensionWalletReviewNotice()}${panel}</div></div>`;
}
  ...
  return `<section class="view-enter about-view">
    <div class="about-surface">
      <div class="about-identity">
        <img src="assets/logo/z00z-logo-gold-circle.png?v=c9778593beb14fabebcae28a55fc3071e797508a" alt="">
...
function statusBadge(status) {
  ...
  const label = stateKeys.has(status) ? t(`plan2.states.${status}`) : titleCase(status);
  return `<span class="status-badge ${className}">${escapeHtml(label)}</span>`;
}

function extensionObjectFamilyChips(families) {
  return `<div class="extension-chip-list">${families.map((family) => `<span class="extension-chip">${escapeHtml(titleCase(family))}</span>`).join("")}</div>`;
}

function extensionCard(entry) {
  return `<article class="extension-card" data-extension-card="${escapeHtml(entry.id)}">
    <div class="extension-card-heading">
      <span class="extension-card-icon" aria-hidden="true">${icon(entry.iconName)}</span>
      <div><p class="eyebrow">${escapeHtml(titleCase(entry.useCaseFamily))}</p><h3>${escapeHtml(entry.label)}</h3></div>
    </div>
    <p>${escapeHtml(entry.summary)}</p>
    <dl class="extension-card-metadata">
      <div><dt>Maturity</dt><dd>${escapeHtml(titleCase(entry.maturity))}</dd></div>
      <div><dt>Availability</dt><dd>${escapeHtml(titleCase(entry.availability))}</dd></div>
      <div><dt>Publisher</dt><dd>${escapeHtml(entry.publisher.label)} · Unverified</dd></div>
    </dl>
    <div class="extension-card-section"><strong>Requested objects</strong>${extensionObjectFamilyChips(entry.requestedObjectFamilies)}</div>
    <div class="extension-card-section"><strong>Offline behavior</strong><p>${escapeHtml(entry.offlineBehavior.summary)}</p></div>
    <div class="extension-card-section"><strong>Data disclosed</strong><p>${escapeHtml(entry.disclosures.map(titleCase).join(" · "))}</p></div>
    <div class="extension-card-actions">
      <button class="button button-primary" type="button" data-extension-action="route" data-extension-route="${escapeHtml(entry.routeId)}">Open interface</button>
      <button class="button button-quiet" type="button" data-extension-action="open" data-extension-id="${escapeHtml(entry.id)}">${escapeHtml(t("plan2.actions.openDetails"))}</button>
      <button class="button button-quiet" type="button" data-help-topic="${escapeHtml(entry.helpTopicId)}">Help ↗</button>
    </div>
  </article>`;
}

function screenHeading(title, copy, meta = "") {
  return `<header class="extension-screen-heading"><div><p class="eyebrow">Local fixture workflow</p><h2>${escapeHtml(title)}</h2><p>${escapeHtml(copy)}</p></div>${meta}</header>`;
}

function extensionDiscoverScreen() {
  return `${screenHeading("Discover Extensions", `${demoRuntime.EXTENSION_CATALOG.length} curated Z00Z typed-action interfaces. Every card is a bundled local descriptor, never remotely executed application code.`, `<span class="status-badge">${demoRuntime.EXTENSION_CATALOG.length} descriptors</span>`)}
    <section class="extension-catalog-grid" aria-label="${escapeHtml(t("plan2.aria.extensionCatalogue"))}">${demoRuntime.EXTENSION_CATALOG.map((entry) => extensionCard(entry)).join("")}</section>`;
}

function extensionProposalField(entry, field) {
  const controlId = `extension-${entry.id}-${field.id}`;
  ...
  return `<section class="wcoins-gateway-overview" aria-label="Stable asset route status">
    ...
    <div class="wcoins-gateway-routes">
      ${entry.gatewayRoutes.map((route) => `<article class="wcoins-route-card" data-wcoins-route="${escapeHtml(route.id)}">
        <header><div><p class="eyebrow">${escapeHtml(route.externalAsset)} reserve · ${escapeHtml(route.reserveNetwork)}</p><h3>${escapeHtml(route.label)}</h3></div>${statusBadge(route.status.toLowerCase())}</header>
...
function extensionXChainOverview(entry) {
...
function clearExtensionProposalValidation(form) {
  [...form.elements]
  ...
  const error = form.querySelector("#extension-proposal-error");
  if (error) error.textContent = "";
}

function validateExtensionProposalForm(form, descriptor) {
  const error = form.querySelector("#extension-proposal-error");
  const controls = [...form.elements].filter((control) => control instanceof HTMLInputElement || control instanceof HTMLSelectElement);
  clearExtensionProposalValidation(form);
  ...
  const values = extensionProposalFormValues(form);
...
function extensionProposalScreen(entry) {
  ...
  return `<section class="extension-proposal" data-extension-proposal="${escapeHtml(entry.id)}" data-intent-type="${escapeHtml(entry.intentType)}">
    <header class="extension-detail-heading">
      <span class="extension-card-icon is-large">${icon(entry.iconName)}</span>
      <div><p class="eyebrow">${escapeHtml(titleCase(entry.useCaseFamily))}</p><h2>${escapeHtml(entry.label)}</h2><p>${escapeHtml(entry.summary)}</p></div>
      ${statusBadge(entry.maturity)}
    </header>
    <div class="extension-architecture-boundary">${icon("shield")}<span><strong>Extension proposes; Wallet decides</strong><small>A Z00Z Extension does not control the wallet. It proposes a typed action. Wallet checks scope, builds the package, requests confirmation, and only then passes it to the settlement path.</small></span></div>
    ${extensionGatewayOverview(entry)}
    ${extensionXChainOverview(entry)}
    <div class="extension-proposal-layout">
      <form class="extension-proposal-form" id="extension-action-proposal-form" data-extension-id="${escapeHtml(entry.id)}" autocomplete="off" novalidate>
        <div class="extension-proposal-form-heading"><div><p class="eyebrow">Typed action</p><h3>Prepare proposal</h3></div><code>${escapeHtml(entry.intentType)}</code></div>
        <dl class="extension-creation-summary">
        ...
        <div class="extension-proposal-fields">${entry.proposalFields.map((field) => extensionProposalField(entry, field)).join("")}</div>
        <p class="field-error extension-proposal-error" id="extension-proposal-error" role="alert"></p>
        ...
        <button class="button button-primary extension-proposal-submit" type="submit">${icon(entry.iconName)} ${escapeHtml(entry.actionLabel)}</button>
      </form>
      <aside class="extension-proposal-review" aria-label="Wallet review boundary">
        <div><p class="eyebrow">Wallet boundary</p><h3>What happens next</h3></div>
        <ol class="extension-proposal-stages">${stages.map(([label, detail], index) => `<li><span>${index + 1}</span><div><strong>${escapeHtml(label)}</strong><small>${escapeHtml(detail)}</small></div></li>`).join("")}</ol>
        <dl class="extension-proposal-output"><div><dt>Requested objects</dt><dd>${escapeHtml(entry.requestedObjectFamilies.map(titleCase).join(", "))}</dd></div><div><dt>Evidence output</dt><dd>${escapeHtml(entry.evidenceOutput)}</dd></div><div><dt>Remote code</dt><dd>Never loaded</dd></div></dl>
...
function extensionDetailScreen() {
  const entry = demoRuntime.extensionDescriptor(state.extensionSelectedId);
  if (!entry) {
    state.extensionScreen = "list";
    return extensionDiscoverScreen();
  }
  const reviewableConnection = demoRuntime.EXTENSION_CONNECTION_FIXTURES.find(({ descriptorId, status }) => descriptorId === entry.id && status === "pending");
  return `<section class="extension-detail" data-extension-detail="${escapeHtml(entry.id)}">
    <button class="button button-quiet extension-back-button" type="button" data-extension-action="back">← Back to ${escapeHtml(titleCase(state.extensionSection))}</button>
    <div class="extension-detail-heading"><span class="extension-card-icon is-large">${icon(entry.iconName)}</span><div><p class="eyebrow">${escapeHtml(titleCase(entry.useCaseFamily))}</p><h2>${escapeHtml(entry.label)}</h2><p>${escapeHtml(entry.summary)}</p></div></div>
    <div class="capability-note">${icon("alert")}<span><strong>${escapeHtml(titleCase(entry.maturity))} · ${escapeHtml(titleCase(entry.availability))}</strong><small>${escapeHtml(entry.reviewBoundary)}</small></span></div>
    <div class="extension-detail-grid">
      <section class="extension-detail-panel"><h3>Trust and execution</h3><dl class="extension-detail-list"><div><dt>Publisher</dt><dd>${escapeHtml(entry.publisher.label)}</dd></div><div><dt>Provenance</dt><dd>${escapeHtml(titleCase(entry.publisher.provenance))}</dd></div><div><dt>Verified</dt><dd>No</dd></div><div><dt>Execution</dt><dd>Typed intent only · no wallet bridge</dd></div></dl></section>
      <section class="extension-detail-panel"><h3>Requested capability</h3><dl class="extension-detail-list"><div><dt>Intent</dt><dd><code>${escapeHtml(entry.intentType)}</code></dd></div><div><dt>Objects</dt><dd>${escapeHtml(entry.requestedObjectFamilies.map(titleCase).join(", "))}</dd></div><div><dt>Value path</dt><dd>${escapeHtml(titleCase(entry.valuePath))}</dd></div><div><dt>Fee path</dt><dd>${escapeHtml(titleCase(entry.feePath))}</dd></div></dl></section>
      <section class="extension-detail-panel"><h3>Offline behavior</h3><p>${escapeHtml(entry.offlineBehavior.summary)}</p><p class="extension-detail-key">${escapeHtml(titleCase(entry.offlineBehavior.mode))}</p></section>
      <section class="extension-detail-panel"><h3>Data disclosed</h3>${extensionObjectFamilyChips(entry.disclosures)}<p>No raw wallet object, seed, key, session, or arbitrary local path is shared.</p></section>
    </div>
    <div class="extension-card-actions">${reviewableConnection ? `<button class="button button-primary" type="button" data-extension-action="review" data-connection-id="${escapeHtml(reviewableConnection.id)}">Review pending request</button>` : ""}<button class="button button-quiet" type="button" data-help-topic="${escapeHtml(entry.helpTopicId)}">Help ↗</button></div>
  </section>`;
}

function extensionReviewScreen() {
  const result = extensionGateway.readPermissionReview({ connectionId: state.extensionReviewConnectionId });
  const review = result.ok ? result.data : null;
  const descriptor = review ? demoRuntime.extensionDescriptor(review.descriptorId) : null;
  if (!review || !descriptor) {
    state.extensionScreen = "list";
    return extensionDiscoverScreen();
  }
  const acknowledgements = state.extensionReviewAcknowledgements || {
    scopeConfirmed: false,
    reauthAcknowledged: false
  };
  return `<section class="extension-review" data-extension-review="${escapeHtml(review.connectionId)}" data-extension-review-id="${escapeHtml(review.reviewId)}">
    <button class="button button-quiet extension-back-button" type="button" data-extension-action="back">← Back</button>
    <header class="extension-detail-heading"><span class="extension-card-icon is-large">${icon(descriptor.iconName)}</span><div><p class="eyebrow">Permission review</p><h2>${escapeHtml(review.appIdentity.label)}</h2><p>${escapeHtml(review.intent.humanReadable)}</p></div></header>
    ...
    <form id="extension-permission-review-form" class="extension-review-form" autocomplete="off" novalidate>
      <dl class="extension-review-grid">
        ...
        <div><dt>${escapeHtml(t("plan2.permission.objectFamily"))}</dt><dd>${escapeHtml(titleCase(review.permission.objectFamily))}</dd></div>
        ...
        <div><dt>${escapeHtml(t("plan2.permission.expiry"))}</dt><dd>${escapeHtml(formatDisplayDateTime(review.permission.expiry))}</dd></div>
      ...
      <section class="extension-review-confirmations" aria-label="${escapeHtml(t("plan2.permission.confirmations"))}">
        ...
        <label class="checkbox-line"><input name="reauthAcknowledged" type="checkbox"${acknowledgements.reauthAcknowledged ? " checked" : ""}><span><strong>${escapeHtml(t("plan2.permission.acknowledgeReauth"))}</strong><small>${review.reauth.required ? "A later value or fee path requires fresh authentication inside Wallet review." : "This intent has no value-bearing Wallet re-auth path."} Extensions never collects the Wallet credential.</small></span></label>
      </section>
      <p class="field-error extension-review-error" id="extension-review-error" role="alert">${escapeHtml(state.extensionReviewValidationError || "")}</p>
      <div class="extension-review-actions"><button class="button button-quiet" type="button" data-extension-action="decide" data-decision="rejected">${escapeHtml(t("plan2.actions.reject"))}</button><button class="button button-primary" type="submit">${escapeHtml(t("plan2.permission.acceptIntent"))}</button></div>
    </form>
  </section>`;
}

function extensionOutcomeScreen() {
  const outcome = state.extensionLastOutcome;
  if (!outcome) {
    state.extensionScreen = "list";
    return extensionDiscoverScreen();
  }
  ...
  const walletReviewAction = accepted && state.extensionReviewDecision?.decision === "accepted"
    ? `<button class="button button-primary" type="button" data-extension-action="wallet-review">Continue in Wallet review</button>`
    : "";
  return `<section class="extension-outcome ${accepted ? "is-positive" : "is-negative"}" data-extension-outcome-route="${escapeHtml(outcome.kind)}">
    ...
    <div class="capability-note">${icon("shield")}<span><strong>Wallet state unchanged</strong><small>This outcome stores presentation state only. An accepted typed intent can continue into a separate Wallet review without granting the Extension mutation authority.</small></span></div>
    <div class="extension-card-actions">${walletReviewAction}<button class="button ${accepted ? "" : "button-primary"}" type="button" data-extension-action="outcome-back" data-return-route="${escapeHtml(outcome.returnRoute)}">Back to ${escapeHtml(titleCase(outcome.returnRoute.split(".").at(-1)))}</button></div>
  </section>`;
}

function extensionsView() {
  const routeSection = state.activeRoute.startsWith("extensions.") ? state.activeRoute.split(".").at(-1) : state.extensionSection;
  state.extensionSection = routeSection;
  let content;
  if (state.extensionScreen === "detail") content = extensionDetailScreen();
  else if (state.extensionScreen === "review") content = extensionReviewScreen();
  else if (state.extensionScreen === "outcome") content = extensionOutcomeScreen();
  else if (demoRuntime.extensionDescriptor(routeSection)) content = extensionProposalScreen(demoRuntime.extensionDescriptor(routeSection));
  else content = extensionDiscoverScreen();
  const helpTopicOverride = state.extensionScreen === "detail"
    ? "extensions.detail"
    : state.extensionScreen === "review" ? "extensions.permission-review" : "";
  return `<section class="view-enter extension-roadmap" data-extension-screen="${escapeHtml(state.extensionScreen)}" data-extension-section="${escapeHtml(routeSection)}"${helpTopicOverride ? ` data-help-topic-override="${helpTopicOverride}"` : ""}>${content}</section>`;
}
  ...
  return `<article class="messenger-message-card${message.severity === "danger" ? " is-danger" : ""}" data-messenger-message="${escapeHtml(message.id)}">
    <button class="messenger-message-open" type="button" data-messenger-action="open" data-message-id="${escapeHtml(message.id)}">
      ...
      <span class="messenger-message-copy">
        <span><strong>${escapeHtml(message.senderLabel)}</strong><small>${escapeHtml(formatDisplayDateTime(message.createdAt))}</small></span>
        <b>${escapeHtml(message.subject)}</b>
        <small>${escapeHtml(message.preview)}</small>
      </span>
      ${statusBadge(isBlocked ? "blocked" : status)}
    </button>
  </article>`;
}
...
function messengerFolderScreen(folder) {
  ...
  return `${screenHeading(title, copy, `<span class="status-badge">${items.length} local items</span>`)}
    ${items.length
...
function messengerConversationScreen() {
  return `${screenHeading("Conversations", "Short-lived off-chain thread previews. Search and retention remain local concepts.", `<span class="status-badge">${demoRuntime.MESSENGER_CONVERSATIONS.length} threads</span>`)}
    ...
    <section class="messenger-record-grid">${demoRuntime.MESSENGER_CONVERSATIONS
      ...
      .map((entry) => `<article class="messenger-record-card" data-messenger-conversation="${escapeHtml(entry.id)}"><div><span class="messenger-avatar">${icon("message")}</span><span><strong>${escapeHtml(entry.label)}</strong><small>${escapeHtml(formatDisplayDateTime(entry.updatedAt))}</small></span></div><p>${escapeHtml(entry.preview)}</p><dl><div><dt>Retention</dt><dd>${escapeHtml(entry.retention)}</dd></div><div><dt>Items</dt><dd>${entry.messageCount}</dd></div></dl><div class="capability-note">${icon("shield")} <span><strong>Concept thread</strong><small>No durable mailbox, public presence, or live relay is connected.</small></span></div></article>`).join("")}</section>`;
}

function messengerSentScreen() {
  return `${screenHeading("Sent", "Sent and transport states are advisory. A failed relay cannot roll back or confirm Wallet state.", `<span class="status-badge">${demoRuntime.MESSENGER_SENT.length} local items</span>`)}
    <section class="messenger-record-grid">${demoRuntime.MESSENGER_SENT.map((entry) => `<article class="messenger-record-card" data-messenger-sent="${escapeHtml(entry.id)}"><div><span class="messenger-avatar">${icon("send")}</span><span><strong>${escapeHtml(entry.subject)}</strong><small>${escapeHtml(formatDisplayDateTime(entry.updatedAt))}</small></span>${statusBadge(entry.state)}</div><p>${escapeHtml(entry.summary)}</p></article>`).join("")}</section>`;
}
...
function messengerMessageDetail() {
  ...
  return `<section class="messenger-detail" data-messenger-detail="${escapeHtml(message.id)}">
    ...
    <header class="extension-detail-heading"><span class="extension-card-icon is-large">${icon(message.kind === "request" ? "receive" : message.kind === "abuse" ? "alert" : "message")}</span><div><p class="eyebrow">${escapeHtml(titleCase(message.kind))}</p><h2>${escapeHtml(message.subject)}</h2><p>${escapeHtml(message.preview)}</p></div>${statusBadge(messengerMessageState(message))}</header>
    <dl class="messenger-detail-grid">
      <div><dt>From</dt><dd>${escapeHtml(message.senderLabel)}</dd></div>
      <div><dt>Received</dt><dd>${escapeHtml(formatDisplayDateTime(message.createdAt))}</dd></div>
      <div><dt>Expires</dt><dd>${escapeHtml(formatDisplayDateTime(message.expiresAt))}</dd></div>
      <div><dt>Delivery</dt><dd>${escapeHtml(titleCase(message.deliveryState))} · advisory only</dd></div>
      ${request ? `<div><dt>Request type</dt><dd>${escapeHtml(titleCase(request.type))}</dd></div><div><dt>Exact scope</dt><dd>${escapeHtml(request.exactScope)}</dd></div>` : ""}
    </dl>
...
function messengerRequestReview() {
  ...
  return `<section class="messenger-review" data-messenger-review="${escapeHtml(review.reviewId)}">
    ...
    <header class="extension-detail-heading"><span class="extension-card-icon is-large">${icon("receive")}</span><div><p class="eyebrow">Advisory request review</p><h2>${escapeHtml(review.subject)}</h2><p>${escapeHtml(review.senderLabel)}</p></div>${statusBadge(review.expired ? "expired" : "pending")}</header>
    ...
    <dl class="extension-review-grid">
      <div><dt>Type</dt><dd>${escapeHtml(titleCase(review.request.type))}</dd></div>
      <div><dt>Object family</dt><dd>${escapeHtml(titleCase(review.request.objectFamily))}</dd></div>
      ...
      <div><dt>Expires</dt><dd>${escapeHtml(formatDisplayDateTime(review.expiresAt))}</dd></div>
...
function messengerOutcomeScreen() {
  ...
  return `<section class="extension-outcome${accepted ? "" : " is-negative"} messenger-outcome" data-messenger-outcome="${escapeHtml(outcome.kind)}">
...
function contactStatusBadge(status) {
  ...
  return `<span class="status-badge ${className}">${escapeHtml(titleCase(status))}</span>`;
}
...
function contactCard(contact) {
  return `<article class="contact-book-entry" data-contact="${escapeHtml(contact.id)}"><button class="contact-book-row" type="button" data-contact-action="open" data-contact-id="${escapeHtml(contact.id)}" aria-label="Open ${escapeHtml(contact.label)} contact">
      ...
      <span class="contact-book-copy"><strong>${escapeHtml(contact.label)}</strong><small>Last used ${escapeHtml(formatDisplayDateTime(contact.lastLocalUseAt))}</small></span>
...
function contactsListScreen() {
  ...
  return `${screenHeading("Address book", "Choose a nickname to open that contact's local details. No presence lookup or contact upload occurs.", `<button class="button button-primary" type="button" data-contact-action="add">${icon("plus")} Add contact</button>`)}
    <form class="contacts-toolbar" id="contacts-search-form" autocomplete="off">
      ...
      <div class="field-group"><label class="field-label" for="contacts-status">Local status</label><select id="contacts-status" name="status" data-contact-status-filter><option value="all"${state.contactsStatus === "all" ? " selected" : ""}>All statuses</option>${demoRuntime.CONTACT_STATUS_IDS.map((status) => `<option value="${escapeHtml(status)}"${state.contactsStatus === status ? " selected" : ""}>${escapeHtml(titleCase(status))}</option>`).join("")}</select></div>
...
function contactDetailScreen() {
  ...
  return `<section class="contact-detail" data-contact-detail="${escapeHtml(contact.id)}">
    ...
    <dl class="contact-detail-grid">
      ...
      <div><dt>Last local use</dt><dd>${escapeHtml(formatDisplayDateTime(contact.lastLocalUseAt))}</dd></div>
      <div><dt>Compatibility</dt><dd>${escapeHtml(contact.compatibility)}</dd></div>
      <div><dt>Expiry</dt><dd>${escapeHtml(formatDisplayDateTime(contact.expiresAt))}</dd></div>
...
function contactImportScreen() {
  ...
  return `<section class="contact-import" data-contact-import="${escapeHtml(source.id)}">
    <button class="button button-quiet" type="button" data-contact-action="back">← Contacts</button>
    ${screenHeading("Add contact", "Choose one reviewed local source. Browser camera, arbitrary URL, remote file, and secret import are unavailable.")}
...
function contactEditScreen() {
  ...
  const contact = result.data.contact;
  return `<section class="contact-edit" data-contact-edit="${escapeHtml(contact.id)}"><button class="button button-quiet" type="button" data-contact-action="detail">← Contact</button>${screenHeading("Edit local label", "This changes only local presentation metadata, never receiver material or counterparty state.")}
...
function contactOutcomeScreen() {
  ...
  return `<section class="extension-outcome contact-outcome" data-contact-outcome="${escapeHtml(outcome.kind)}"><span class="result-icon">${icon(outcome.kind === "removed" ? "remove" : "check")}</span><p class="eyebrow">Local contact outcome</p><h2>${escapeHtml(outcome.title)}</h2><p>${escapeHtml(outcome.summary)}</p><div class="capability-note">${icon("shield")}<span><strong>No remote side effect</strong><small>No contact upload, public presence, implicit trust, Wallet mutation, settlement mutation, or protocol revocation occurred.</small></span></div><button class="button button-primary" type="button" data-contact-action="back">Back to contacts</button></section>`;
}
...
function completeExtensionPermissionReview(decision, acknowledgements = {}) {
  const reviewResult = extensionGateway.readPermissionReview({
    connectionId: state.extensionReviewConnectionId
  });
  if (!reviewResult.ok) {
    state.extensionReviewValidationError = reviewResult.error.message;
    render();
    requestAnimationFrame(() => document.querySelector("#extension-review-error")?.focus?.());
    return false;
  }

  const result = extensionGateway.decidePermissionReview({
  ...
  if (!result.ok) {
    state.extensionReviewValidationError = result.error.message;
    state.extensionReviewAcknowledgements = {
      scopeConfirmed: Boolean(acknowledgements.scopeConfirmed),
      reauthAcknowledged: Boolean(acknowledgements.reauthAcknowledged)
    };
    render();
  ...
  state.extensionReviewValidationError = null;
  state.extensionReviewDecision = result.data;
  state.extensionLastOutcome = {
    ...
    returnRoute: "extensions.discover",
    descriptorId: result.data.descriptorId
  };
  state.extensionScreen = "outcome";
  render({ focusMain: true });
...
document.addEventListener("click", (event) => {
  ...
  if (extensionActionButton) {
    const action = extensionActionButton.dataset.extensionAction;
    if (action === "route") {
      const routeId = extensionActionButton.dataset.extensionRoute;
      if (!demoRuntime.PORT_CONTRACT.extensionRoutes.includes(routeId)) {
        showToast("Unknown local extension route.", "alert");
        return;
      }
      selectCanonicalRoute(routeId);
      render({ focusMain: true });
    } else if (action === "open") {
      const descriptor = demoRuntime.extensionDescriptor(extensionActionButton.dataset.extensionId);
      if (!descriptor) {
        showToast("Unknown local extension descriptor.", "alert");
        return;
      }
      state.extensionSelectedId = descriptor.id;
      state.extensionReviewConnectionId = null;
      state.extensionReviewValidationError = null;
      state.extensionScreen = "detail";
      render({ focusMain: true });
    } else if (action === "back") {
      state.extensionScreen = "list";
      state.extensionSelectedId = null;
      state.extensionReviewConnectionId = null;
      state.extensionReviewValidationError = null;
      state.extensionReviewAcknowledgements = {
        scopeConfirmed: false,
      ...
      const connection = demoRuntime.EXTENSION_CONNECTION_FIXTURES.find(({ id }) => id === extensionActionButton.dataset.connectionId);
      if (!connection) {
        showToast("Unknown local connection fixture.", "alert");
        return;
      }
      clearExternalReviewHandoffs();
      state.extensionSelectedId = connection.descriptorId;
      state.extensionReviewConnectionId = connection.id;
      state.extensionReviewValidationError = null;
      state.extensionReviewAcknowledgements = {
        scopeConfirmed: false,
        reauthAcknowledged: false
      };
      state.extensionScreen = "review";
      render({ focusMain: true });
    } else if (action === "decide") {
      const decision = extensionActionButton.dataset.decision;
      if (decision !== "rejected") {
        showToast("The local review decision failed closed.", "alert");
        return;
      }
      clearExternalReviewHandoffs();
      completeExtensionPermissionReview("rejected");
    } else if (action === "wallet-review") {
      const handoffResult = extensionGateway.prepareWalletReview({
        decision: state.extensionReviewDecision
      });
      if (!handoffResult.ok) {
      ...
      clearExternalReviewHandoffs();
      state.extensionWalletReviewHandoff = handoff;
      ...
      render({ focusMain: true });
      showToast("Typed extension intent opened in Wallet review.");
    } else if (action === "outcome-back") {
      selectCanonicalRoute(extensionActionButton.dataset.returnRoute);
      render({ focusMain: true });
    }
    return;
  }
  ...
  if (messengerActionButton) {
    ...
    if (action === "open") {
      ...
      render({ focusMain: true });
      showToast(`${titleCase(gatewayAction)} locally; Wallet state unchanged.`);
  ...
  if (contactActionButton) {
    ...
    if (action === "open") {
      ...
      selectCanonicalRoute(handoff.target.routeId);
      render({ focusMain: true });
      showToast(`${titleCase(action)} opened as a typed Messenger concept.`);
    }
    return;
  }
...
document.addEventListener("submit", (event) => {
  event.preventDefault();
  if (event.target.id === "extension-action-proposal-form") {
    const descriptor = demoRuntime.extensionDescriptor(event.target.dataset.extensionId);
    if (!descriptor) {
      showToast("Unknown local extension descriptor.", "alert");
      return;
    }
    if (!validateExtensionProposalForm(event.target, descriptor)) return;
    state.extensionSelectedId = descriptor.id;
    state.extensionLastOutcome = {
  ...
  } else if (event.target.id === "extension-permission-review-form") {
    ...
    state.extensionReviewAcknowledgements = acknowledgements;
    completeExtensionPermissionReview("accepted", acknowledgements);
...
document.addEventListener("input", (event) => {
  const extensionProposalForm = event.target instanceof Element
    ? event.target.closest("#extension-action-proposal-form")
    : null;
  if (extensionProposalForm instanceof HTMLFormElement) clearExtensionProposalValidation(extensionProposalForm);
