...
((root) => {
  ...
  if (!demo?.PORT_CONTRACT || !demo.EXTENSION_CATALOG || !demo.EXTENSION_CONNECTION_FIXTURES) {
    throw new Error("Z00Z contracts and the local extension catalogue must load before the mock extension gateway.");
  }
  ...
  function reviewFromConnection(connection) {
    const descriptor = demo.extensionDescriptor(connection.descriptorId);
    const valuePresent = hasValue(connection.value);
    const feePresent = hasValue(connection.fee);
    return deepFreeze({
      schemaVersion: "extension-permission-review-v1",
  ...
  const EXTENSION_PERMISSION_REVIEWS = deepFreeze(
    demo.EXTENSION_CONNECTION_FIXTURES.map(reviewFromConnection)
  );
  const REVIEW_BY_ID = new Map(EXTENSION_PERMISSION_REVIEWS.map((review) => [review.reviewId, review]));
  const REVIEW_BY_CONNECTION_ID = new Map(EXTENSION_PERMISSION_REVIEWS.map((review) => [review.connectionId, review]));

  function proposalFromReview(review) {
    return deepFreeze({
      schemaVersion: "extension-intent-proposal-v1",
  ...
  function heldAuthorityFromReview(review) {
    return deepFreeze({
      schemaVersion: "extension-held-authority-v1",
      connectionId: review.connectionId,
      permission: {
        objectFamily: review.permission.objectFamily,
        exactScope: review.permission.exactScope,
  ...
  const EXTENSION_INTENT_PROPOSALS = deepFreeze(EXTENSION_PERMISSION_REVIEWS.map(proposalFromReview));
  const EXTENSION_HELD_AUTHORITIES = deepFreeze(EXTENSION_PERMISSION_REVIEWS.map(heldAuthorityFromReview));
  const PROPOSAL_BY_CONNECTION_ID = new Map(EXTENSION_INTENT_PROPOSALS.map((proposal) => [proposal.connectionId, proposal]));
  const HELD_AUTHORITY_BY_CONNECTION_ID = new Map(EXTENSION_HELD_AUTHORITIES.map((authority) => [authority.connectionId, authority]));
  ...
  function validateIntentProposal({ proposal, heldAuthority } = {}) {
    ...
    if (!demo.EXTENSION_INTENT_TYPES.includes(proposal.intent?.type)) {
    ...
    const descriptor = demo.extensionDescriptor(String(proposal.descriptorId || ""));
    ...
    if (!exactKeys(proposal, ["schemaVersion", "connectionId", "descriptorId", "intent", "permission", "value", "fee", "boundary"])
      || proposal.schemaVersion !== "extension-intent-proposal-v1"
    ...
    if (!heldAuthority
      || heldAuthority.schemaVersion !== "extension-held-authority-v1"
    ...
    return ok({
      schemaVersion: "extension-intent-validation-v1",
  ...
  function createMockExtensionGateway() {
    ...
    function decidePermissionReview({
      ...
      return ok({
        schemaVersion: "extension-permission-decision-v1",
    ...
    function prepareWalletReview({ decision, proposal, heldAuthority } = {}) {
      ...
      if (!review
        || decision?.schemaVersion !== "extension-permission-decision-v1"
        ...
        return fail("invalid_decision", "The accepted extension decision failed Wallet handoff validation.");
      }
      ...
      return ok({
        schemaVersion: "extension-wallet-review-handoff-v1",
    ...
    return Object.freeze({
      ...
      reviewSchemaVersion: "extension-permission-review-v1",
      decisionSchemaVersion: "extension-permission-decision-v1",
  ...
  Object.assign(root.Z00ZDemo, {
    ...
    EXTENSION_INTENT_PROPOSALS,
    EXTENSION_HELD_AUTHORITIES,
    WALLET_REVIEW_TARGETS,
    validateExtensionIntentProposal: validateIntentProposal,
    createMockExtensionGateway
  });
