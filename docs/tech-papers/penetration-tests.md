# 066 Local Pentest Orchestration TODO

Status: authoritative Phase 066 implementation backlog  
Last verified against local inputs and upstream sources: 2026-06-30  
Adversarial recheck status: revalidated against current upstream HEADs on 2026-06-30  
Audience: implementers building reusable Codex/Copilot local security orchestration  
Primary source note: `.planning/phases/066-Strix/strix.md` is an input, not the final authority after this file is accepted.

This file defines the full implementation plan for a local-only, coder-agnostic penetration-testing orchestration system inspired by Strix and HexStrike, but implemented as repository-local Codex/GitHub Copilot skills, scripts, agents, prompts, tool installers, and portable restore hooks.
...
./scripts/run_pentest_tools.sh = canonical human/agent entrypoint, including --docker-sandbox
...
| Source ID | Source | Evidence Used | Implementation Meaning |
| --- | --- | --- | --- |
| `SRC-002` | `https://github.com/usestrix/strix` at commit `f342808d2b4af551a17e47af744d2f19dee3c443` | `README.md`, `pyproject.toml`, `strix/skills/**` including `coordination`, `custom`, `tooling`, `scan_modes`, `frameworks`, `protocols`, `technologies`, and `vulnerabilities` | Strix requires Docker and LLM API keys as a product, but its skill corpus is useful as a reference library. |
| `SRC-003` | `https://github.com/0x4m4/hexstrike-ai` at commit `9b8c780f324ce5145a322bfa23c98886f8424ba3` | `README.md`, `hexstrike_mcp.py`, `hexstrike_server.py`, `requirements.txt`, `LICENSE` | HexStrike is an MCP framework with many tool wrappers; use only allowlisted tool inventory, timeout patterns, and safe local CLI ideas. |
| `SRC-004` | `.github/skills/attack-surfaces-create/SKILL.md` and `REFERENCE.md` | Inventory-first attack-surface audit method | Reuse for source-backed attack surface mapping and candidate admission. |
| `SRC-005` | `.github/skills/z00z-crypto-auditor/SKILL.md` and `FORMS.md` | Z00Z crypto/security audit workflow | Reuse as the Z00Z-specific protocol audit adapter. |
| `SRC-006` | `.github/prompts/gsd-audit-4.prompt.md` | Four-pass audit and doublecheck closure model | Reuse as the heavy closeout/audit prompt for phase-level security closure. |
| `SRC-007` | `pack_z00z_project.sh` and `unpack_z00z_project.sh` | Existing portable archive, Docker bootstrap fragments, symlink manifest, restore verification | Reuse packaging and symlink-check ideas, but do not use `unpack_z00z_project.sh --docker-sandbox` as-is for pentests because it installs and runs formal-verification flows. |
| `SRC-008` | `.codex/skills -> ../.github/skills` and related `.codex` symlinks | Existing local symlink model | New skills must live in `.github/skills`; `.codex` must expose them through symlinked surfaces. |
...
- `REQ-016`: Pentest Docker workflows must not install, bootstrap, or run formal-verification tooling. Do not call `scripts/verification-tools/install-verification-tools.sh`, `tools/formal_verification/**`, `z00z-full-verify-gate`, or verification-orchestrator report flows from the pentest Docker path.
- `REQ-017`: `./scripts/run_pentest_tools.sh --docker-sandbox` must be the canonical Docker entrypoint for penetration tests. It must be purpose-built for pentests and must not delegate to `unpack_z00z_project.sh --docker-sandbox`.
...
- `REQ-019`: Final human-facing pentest reports must be generated under `reports/z00z-pentests-report-<timestamp>/` on the host. `.security-artifacts/<timestamp>/` remains the machine-readable artifact tree and must reference the exported host report location.
- `REQ-020`: Docker pentest runs must stream container stdout/stderr to the invoking terminal and must copy every report they generate back to the matching host `reports/z00z-pentests-report-<timestamp>/` directory before exit.
...
reports/
  z00z-pentests-report-<timestamp>/
```
...
| HexStrike `README.md` tool lists | Extract allowlist, optional-lab list, and denylist | `.github/skills/pentest-tools-installer/references/hexstrike/tool-inventory.md` | Preserves category value without importing unsafe execution wrappers. |
| HexStrike `hexstrike_mcp.py` | Copy only as reference if needed, with warning header | `.github/skills/pentest-tools-installer/references/hexstrike/hexstrike_mcp_reference_only.py` | Useful for tool name inventory; must never be run. |
...
| Rust verification | `cargo fmt`, `cargo clippy`, `cargo test`, `cargo nextest`, selective `cargo miri`, selective `cargo fuzz` |
| Local DAST | `nmap`, `nuclei`, `httpx`, `katana`, `ffuf`, `gobuster` |
| Optional bounded local-only DAST | `naabu`, `dirsearch`, `nikto` only when scope validation, rate limits, and local targets are enforced |
| Optional web/frontend SAST | `retire`, `eslint` security rules, package-manager audit commands only when the repository contains matching package manifests |
| Containers/IaC | `trivy config`, optional `checkov` for repo-local config only |
| Reporting | local Python scripts under `scripts/penetration/` |

### ⚠️ Reference-Only Or Future Lab Profile

| Class | Tools Or Behaviors |
| --- | --- |
| SQL injection automation | `sqlmap` may be documented as reference-only; no dumping, OS shell, destructive flags, or default execution path. |
| Public recon and domain enumeration | `subfinder`, `amass`, `theHarvester`, Shodan, Censys, HIBP, breach/leak APIs, and similar tools require a separate explicit authorized-target profile. |
| High-speed discovery | `masscan`, `rustscan`, broad `naabu`, `autorecon`, and wide CIDR workflows are not default and require explicit lab authorization. |
| Browser/proxy automation | Strix `agent_browser`, Caido SDK flows, OAST/interactsh, and proxy replay workflows are reference-only unless a local lab profile explicitly enables them. |

### 🚫 Forbidden By Default

| Class | Tools Or Behaviors |
| --- | --- |
| MCP/API runtime | HexStrike MCP, HexStrike server, Strix runtime, external LLM API keys |
| Credential attacks | `hydra`, `john`, `hashcat`, `medusa`, `patator`, credential stuffing, password cracking |
| Network exploitation and harvesting | `responder`, `netexec`, `crackmapexec`, credential harvesting, SMB exploitation, poisoners |
| Exploit frameworks | Metasploit, `msfvenom`, `pwntools` exploitation runners, exploit generation, payload generation, evasion payloads |
| Public recon | wide internet recon, Shodan/Censys/API OSINT, masscan outside explicit local lab scope |
| Automated exploitation | `commix`, `tplmap`, destructive `sqlmap` modes, AI exploit generation, AI payload generation, advanced payload evasion |
| Dangerous file/system wrappers | arbitrary command execution, create/modify/delete wrappers copied from HexStrike |
| Destructive activity | data exfiltration, persistence, malware, destructive exploitation |
...
Goal:
Define the local-only authorized testing boundary that every skill, script, and agent must enforce.
...
- `.github/skills/pentest-tools-installer/SKILL.md`
...
- `.github/skills/pentest-tools-installer/references/hexstrike/tool-inventory.md`
...
- `.github/skills/pentest-tools-installer/`
...
- Scripts must derive one shared run timestamp and use it for both `.security-artifacts/<timestamp>/` and `reports/z00z-pentests-report-<timestamp>/`.
...
- `build_pentest_report.py` must create a preliminary report without exposing secrets and must write the final report set under `reports/z00z-pentests-report-<timestamp>/`.
...
- `scripts/penetration/run_local_pentest.sh --static-only --mode quick --profile generic` also creates `reports/z00z-pentests-report-<timestamp>/` for the same run.
...
```text
reports/z00z-pentests-report-<timestamp>/
  security-report.md
  ...
```
...
- The canonical host report root is `reports/z00z-pentests-report-<timestamp>/`; every generated Markdown, HTML, JSON, or other human-facing report artifact must be written there.
...
- The artifact manifest records the paired host report directory `reports/z00z-pentests-report-<timestamp>/`.
...
- Add `./scripts/run_pentest_tools.sh` as the canonical local and Docker entrypoint for penetration tests.
- `./scripts/run_pentest_tools.sh --docker-sandbox` must call the pentest-only Docker path, not `unpack_z00z_project.sh --docker-sandbox`.
- `./scripts/run_pentest_tools.sh --docker-sandbox` must create or accept a `pack_z00z_project.sh` tarball artifact and pass that archive into `tools/penetration/docker/run_pentest_container.sh`.
...
- Docker runs must allocate or accept the paired host report directory `reports/z00z-pentests-report-<timestamp>/`, mount it for output, and copy every report generated in the container into that directory before exit.
...
- `./scripts/run_pentest_tools.sh` must forward common flags to `scripts/penetration/run_local_pentest.sh`: `--mode quick|standard|deep`, `--scope`, `--static-only`, `--no-dast`, `--profile generic|z00z`, `--artifact-dir`, and `--check-only`.
...
- The pentest restore/container path must skip `scripts/verification-tools/install-verification-tools.sh`, `tools/formal_verification/**`, `.github/skills/z00z-full-verify-gate/scripts/full_verify.sh`, and `.github/skills/z00z-verification-orchestrator/scripts/orchestrate.sh`.
...
- `./scripts/run_pentest_tools.sh --docker-sandbox --archive /tmp/z00z-pentest-plan-check.tar.gz --mode quick --static-only --check-only` runs the pentest-only Docker check path from the packed artifact.
- `./scripts/run_pentest_tools.sh --docker-sandbox --mode quick --static-only --check-only` creates a fresh packed artifact before launching Docker when `--archive` is not provided.
...
- Docker report runs leave exported host reports under `reports/z00z-pentests-report-<timestamp>/`.
...
- Add `./scripts/run_pentest_tools.sh` with `--docker-sandbox` support specifically for penetration tests.
- Add `--archive <path>` support to `./scripts/run_pentest_tools.sh --docker-sandbox`.
- When `--docker-sandbox` is used without `--archive`, `./scripts/run_pentest_tools.sh` must call `./pack_z00z_project.sh --output <tmp-artifact>` first, then run Docker from that artifact.
...
- `./scripts/run_pentest_tools.sh --docker-sandbox` must mount the packed artifact read-only, extract it inside the container, and write results only to the configured `.security-artifacts/<timestamp>/` output mount.
- `./scripts/run_pentest_tools.sh --docker-sandbox` must also mount the paired host report directory `reports/z00z-pentests-report-<timestamp>/` and copy all generated reports there before the container exits.
...
- `./scripts/run_pentest_tools.sh --mode quick --static-only --check-only` works without Docker.
- `./scripts/run_pentest_tools.sh --docker-sandbox --archive /tmp/z00z-pentest-plan-check.tar.gz --mode quick --static-only --check-only` works with the pentest-only container path.
- `./scripts/run_pentest_tools.sh --docker-sandbox --mode quick --static-only --check-only` produces a temporary packed artifact and records its path in the run manifest.
...
- Pentest Docker output is visible in the invoking terminal and generated reports are present on the host under `reports/z00z-pentests-report-<timestamp>/`.
...
./scripts/run_pentest_tools.sh --docker-sandbox --archive /tmp/z00z-pentest-portable.tar.gz --mode quick --static-only --check-only
...
- `./scripts/run_pentest_tools.sh` exists as the canonical pentest entrypoint and supports `--docker-sandbox` without delegating to `unpack_z00z_project.sh --docker-sandbox`.
- `./scripts/run_pentest_tools.sh --docker-sandbox` runs from a `pack_z00z_project.sh` artifact, either supplied through `--archive` or created before Docker launch.
- Pentest Docker workflow, if implemented, does not install or run formal-verification tooling.
- Final reports are generated under `reports/z00z-pentests-report-<timestamp>/`, and Docker-generated reports are copied back there on the host.
