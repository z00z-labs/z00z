...
use super::plonky3_epoch_jmt_air::{
    first_row_preprocessed, jmt_chunk_npo_type, jmt_npo_type, JmtAirV2, JmtChunkAirV2,
    JmtChunkTraceV2, JmtTraceV2, CALL_FIELDS_V2, CHUNK_PUBLIC_FIELDS_V2, CHUNK_ROW_FIELDS_V2,
    PUBLIC_FIELDS_V2,
};
...
#[derive(Clone, Copy, Debug)]
pub(super) struct JmtChunkProverV2;

impl JmtChunkProverV2 {
    fn batch_instance(
        &self,
        packing: &TablePacking,
        traces: &Traces<KoalaBear>,
    ) -> Option<BatchTableInstance<Plonky3StarkConfigV2>> {
        let trace = traces.non_primitive_trace::<JmtChunkTraceV2>(&jmt_chunk_npo_type())?;
        if trace.rows.is_empty()
            || trace.public_values.len() != CHUNK_PUBLIC_FIELDS_V2
            || trace
                .rows
                .iter()
                .any(|row| row.values.len() != CHUNK_ROW_FIELDS_V2)
        {
            return None;
        }
        let preprocessed = first_row_preprocessed(trace.rows.len());
        let min_height = packing.min_trace_height().max(trace.rows.len());
        let air = JmtChunkAirV2::<KoalaBear, 1>::new(preprocessed, min_height);
        Some(BatchTableInstance {
            op_type: jmt_chunk_npo_type(),
            air: DynamicAirEntry::new(Box::new(air)),
            trace: JmtChunkAirV2::<KoalaBear, 1>::trace_to_matrix(&trace.rows, min_height),
...
impl TableProver<Plonky3StarkConfigV2> for JmtChunkProverV2 {
    fn op_type(&self) -> NpoTypeId {
        jmt_chunk_npo_type()
    }
    ...
    fn batch_air_from_table_entry(
        ...
        if degree != 1
            ...
            || entry.rows == 0
            || entry.lanes != 1
            || entry.public_values.len() != CHUNK_PUBLIC_FIELDS_V2
        {
            return Err("epoch JMT chunk table shape mismatch".into());
        }
        Ok(DynamicAirEntry::new(Box::new(
            JmtChunkAirV2::<KoalaBear, 1>::new(Vec::new(), entry.rows),
        )))
    }
    ...
    fn air_with_committed_preprocessed(
        ...
        (lanes == 1
            && circuit_extension_degree == 1
            && !committed.is_empty()
            && committed.first() == Some(&KoalaBear::ONE)
            && committed[1..].iter().all(|&value| value == KoalaBear::ZERO))
        .then(|| {
            DynamicAirEntry::new(Box::new(JmtChunkAirV2::<KoalaBear, 1>::new(
                committed, min_height,
            )))
        })
    }
