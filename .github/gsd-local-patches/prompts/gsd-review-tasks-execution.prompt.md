...
- In Codex, Copilot, and other inline workspace runtimes, satisfy this prompt by opening this file and executing it inside the current session against the live workspace.
- Do not invoke `gsd`, `codex`, `claude`, `gemini`, `opencode`, `openrouter`, `openai`, or any external CLI, API, or model router to satisfy this prompt.
- If a plan or verify block says to run this prompt at least 3 times in YOLO mode, that means at least 3 distinct inline local review passes on the current workspace, with fixes between passes as needed, until at least 2 consecutive passes report no significant issues.
- A provider-quota, cooldown, or prompt-token explanation is not a valid blocker for this prompt in Codex; it indicates that a non-canonical execution path was attempted.
