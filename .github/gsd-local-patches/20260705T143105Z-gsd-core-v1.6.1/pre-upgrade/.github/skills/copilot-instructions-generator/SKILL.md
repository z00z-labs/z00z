...
# Agent Instructions Blueprint Generator

## When to Use

- The user wants a new `agent-instructions.md` or equivalent repository instruction file built from existing repository patterns.
- A project needs assistant instructions tied to real stack versions, architecture, and local conventions.
- The task is to generate or upgrade repository-specific assistant guidance without guessing unsupported patterns.
- The output should be a reusable blueprint for future code generation, not a one-off code answer.
...
2. **Context Files**: Prioritize patterns and standards defined in the repository's assistant context directories (for example `.github/instructions/`)
...
Prioritize the following files in the repository's assistant context directory (for example `.github/instructions/`) if they exist:
...
To create the repository instruction file, first analyze the codebase to:
...
The final repository instruction file should:
- Be placed in the repository's documented assistant context directory
...
- Be comprehensive yet concise enough for an AI coding assistant to use effectively

Important: Only include guidance based on patterns actually observed in the codebase. Explicitly instruct the assistant to prioritize consistency with existing code over external best practices or newer language features.
"

## Expected Output

A comprehensive repository instruction file that will guide an AI coding assistant to produce code that is perfectly compatible with your existing technology versions and follows your established patterns and architecture.
