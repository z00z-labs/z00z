...
**Line Overlap Prevention** (CRITICAL):
...
**Recommended: Incremental Line-by-Line Generation**:

Build the SVG as a list of individual lines, then join and write at the end. This approach works in any language and prevents truncation, typos, and syntax errors — each line is independent and easy to verify.
...
const lines = [];
lines.push('<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 960 700">');
// ...
lines.push('</svg>');
fs.writeFileSync('/path/to/output.svg', lines.join('\n'));
```

**Pre-Generation Checklist** (use EVERY time):
1. ✅ Is the COMPLETE SVG content ready to write?
2. ✅ Are all shape coordinates and IDs fully resolved?
3. ✅ Have I checked for syntax errors in the prepared content?

**If ANY answer is NO**: STOP. Prepare the content fully before writing.

**Error Recovery Protocol**:
- **First error**: Analyze root cause, apply targeted fix
- **Second error**: Switch method entirely (e.g., switch to chunked or template-based generation)
- **Third error**: STOP and report to user — do NOT loop endlessly
- **Never**: Retry the same failing approach without changing the strategy
