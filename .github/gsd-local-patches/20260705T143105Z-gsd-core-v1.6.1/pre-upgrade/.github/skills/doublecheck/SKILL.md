---
name: doublecheck
description: 'Run a three-layer verification pipeline on AI-generated output using workspace-first evidence, source checks, and adversarial review. Use when the user asks to verify a prior response, doublecheck factual claims, validate citations, audit a summary, or produce a full hallucination-risk report with independent sources for repo-specific or external claims.'
---
...
- The user asks to verify, doublecheck, audit, or fact-check a previous answer.
- A response contains repo-specific claims that should be checked against workspace files first.
- A summary, analysis, or citation-heavy answer needs source-backed validation.
- The user wants a structured hallucination-risk report instead of an informal confidence statement.

## Workspace-First Rule

If the target claims depend on repository files, local plans, code, tests, docs, or any other workspace artifact, verify those claims against workspace files before using web search.

- Use workspace read/search access to inspect the exact files referenced by the request.
- Treat workspace files as the primary evidence source for repo-specific claims.
- Use web search only to validate external facts that are not fully resolved by local repository evidence.
- If workspace-backed claims cannot be checked because read/search access is unavailable, report the verification as blocked by missing local file access instead of marking claims verified, plausible, or contradicted.
- For mixed cases, split the report into local claims and external claims so the user can see which parts were checked against the repo and which parts were checked against the web.
...
0. **Choose the evidence source first.** If the claim is repo-specific, search the workspace before doing any web search. Only use the web for claims whose truth is external to the repository.
