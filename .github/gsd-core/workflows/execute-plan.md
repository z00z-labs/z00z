...
<step name="plan_review_gate">
Review the completed plan's source changes and auto-fix non-clean findings.

Compute the current plan summary path and scope review to the files listed in its frontmatter:

```bash
SUMMARY_PATH="${PHASE_DIR}/${PHASE}-SUMMARY.md"
REVIEW_FILES_OVERRIDE=$(SUMMARY_PATH="${SUMMARY_PATH}" node -e "
  const fs = require('fs');
  const content = fs.readFileSync(process.env.SUMMARY_PATH, 'utf-8');
  ...
  for (const line of fm.split('\n')) {
    const inline = line.match(/^\s+(created|modified):\s*\[(.*)\]\s*$/);
    if (inline) {
      inline[2].split(',').map(f => f.trim()).filter(Boolean).forEach(f => files.push(f));
      continue;
    }
  ...
  const deduped = [...new Set(files)].filter(Boolean);
  if (deduped.length) console.log(deduped.join(','));
")
```

If REVIEW_FILES_OVERRIDE is empty:
```
No source files changed in Plan ${PHASE}. Skipping plan review.
```
Exit this step and continue.

Invoke review for the current plan only:
```
Skill(skill="gsd-code-review", args="${PHASE} --files=${REVIEW_FILES_OVERRIDE}")
```

Check the plan-scoped review artifact:
```bash
REVIEW_PATH="${PHASE_DIR}/${PHASE}-REVIEW.md"
REVIEW_STATUS=$(sed -n '/^---$/,/^---$/p' "$REVIEW_PATH" | grep "^status:" | head -1 | cut -d: -f2 | tr -d ' ')
```

If REVIEW_STATUS is not "clean" and not "skipped" and not empty, display:
```
Code review found issues. Auto-running fix:
/gsd-code-review-fix ${PHASE} --all --auto
```

Invoke:
```
Skill(skill="gsd-code-review-fix", args="${PHASE} --all --auto")
```

**Error handling:** If the fix Skill invocation fails or throws, catch the error, display `Code review fix encountered an error (non-blocking): {error}` and continue. Fix failures must never block execution flow.

**Error handling:** If the review Skill invocation fails or throws, catch the error, display `Code review encountered an error (non-blocking): {error}` and continue. Review failures must never block execution.
</step>
